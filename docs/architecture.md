...
┌──────────────────────────────────────────────────────┐
│                   前端层 (React 18 + Vite)              │
│  Dashboard │ Market │ Strategy │ Backtest │ Sim │ Factor │
└──────────────────────┬───────────────────────────────┘
                       │ HTTP / WebSocket
┌──────────────────────▼───────────────────────────────┐
│                  API层 (FastAPI)                       │
...
┌──────────────────────▼───────────────────────────────┐
│             异步任务层 (Celery + Redis)                 │
│  DataWorker │ BacktestWorker │ FactorWorker │ SimWk   │
...
| 数据源 | `data_update_state`, `data_source_config` | 数据更新状态追踪 |
...
| 因子 | `factor_definitions/values/scoring_rank/analysis` | 因子四表 |
...
| compute-daily-factors | 每日 17:30 | 计算因子值 |
...
1. **统一 PostgreSQL**：所有数据（市场数据/因子/模拟交易/系统状态）存储在一个 PostgreSQL 中，避免多存储维护
2. **三层数据回退**：AData → Baostock → AkShare 自动故障切换，`data_update_state` 表追踪增量拉取进度
